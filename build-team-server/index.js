...
app.post("/api/suggestedTeam:id", (req, res) => {
    const {id} = req.params
    formData = [];
    suggestedTeam = [];
    refactorlist = [];
    ...
    const empBudget = req.body.projectBudget
    const empDuration = req.body.projectDuration

    const update = async () => {
        connection.then((value) => {
            value.query(
                "UPDATE heroku_1aabc12bcbbe678.employers SET budget = ?, duration = ? WHERE userId = ?",
                [empBudget, empDuration, id],
                (err, response) => {
                    if (err) {
                        console.log(err.message);
                    } else {
...
app.post("/api/declineTeam:id", (req, res) => {
    ...
    refactor(suggestedTeam, refactorlist, budget, duration, connection)
    const result = () => {
...
app.get("/api/empteam:id", (req, res) => {
    const { id } = req.params;
    const team = async (res) => {
        connection.then((value) => {
            value.query(
